(** Spamtacus provides tools to compose a customizable
    machine-learning filter for mails. A filter is defined by a {b feature
    vector} (see {!FEATURE} for feature definition) and a {b classify} function
    (see {!DT}). The {!Filter} functor takes care of putting together these
    pieces provided by the user to produce a functionning filter.

    This filter abstraction has been built based on supervized learning model.
    Namely, a mail is classified based on examples: a set of labelled mails
    called the training set. The filter then works in two phases (1) training
    phase (2) labelling phase.

    The {b training phase} is done once to process the training set and build a
    database extracting from the training phase the required features. For
    example, the database of a naive Bayesian filter contains the most frequent
    words of the training set with their frequency in spams and hams.
    ...
    {!Spamtacus} has been written to be included in a MirageOS unikernel. For
    this reason, mails are supposed to be provided through a stream of type
    [unit -> partial option Lwt.t] (see {!Filter.instanciate}) and features are
    extracted chunk by chunk (see {!FEATURE.partial_extract}).  *)
...
(** A feature describes what part of a mail is extracted and processed
    to rank it, as well as how to extract and the algorithm used to process it.
    ...
    Each feature also defines its own database type (see {!FEATURE.db}) and
    functions to read/write it (see {!FEATURE.write_db} and
    {!FEATURE.read_db}).
*)
module type FEATURE = sig
  ...
  (** [partial_extract header chunk] defines how to extract the
      important parts of a [chunk], depending of the corresponding [header].
      The extracted strings are combined to form the feature by [add_partial].

      The input headers should not be used to extract some header values from
      it as each header will be passed multiple times for every chunks
  ...
  (** [extract_from_header_tree (headers, tree)] can be used to extract
      feature from [headers]. [tree] describes the structure of the mail and
      can be traversed to access headers of multipart mail. *)
...
module type FILTER = sig
  ...
  (** [train_and_write_to_file training_set ~output] computes the
      combined database of each feature in [Features] from the [training_set]
      and writes it in [output]. *)
  ...
  (** [instanciate ~input_dir stream (header, tree)] ranks each feature
      of the functor parameter [Features] from an incoming mail [stream] mail
      and its header [(header, tree)]. The [input_dir] must contain the
      dabatase built with {!train_and_write_to_file}. *)
...
module Filter (Features : FV) (DecisionTree : DT) : FILTER
